...
void Engine::DecalSystem::UpdateBuffer()
{
    ...
    {
        instanceCount += m_perTexture[i].instances.size();
    }
    ...
    for (size_t i = 0; i < m_perTexture.size(); i++)
    {
        ...
        {
            dst[copiedNum++] = m_perTexture[i].instances[j];
        }
    }
}
...
void Engine::DecalSystem::AddDecal(const Materials::DecalMaterial& material, const mat4& decalToWorld, uint32_t objectID)
{
    ...
    Instances::DecalInstance instance = { decalToWorld, mat4::Inverse(decalToWorld), objectID };
...
void Engine::DecalSystem::Draw()
{
    ...
    for (size_t i = 0; i < m_perTexture.size(); i++)
    {
        ...
        D3D::GetInstance()->GetContext()->DrawIndexedInstanced(36u, m_perTexture[i].instances.size(), 0, 0, renderedInstances);
        renderedInstances += m_perTexture[i].instances.size();
    }
}
